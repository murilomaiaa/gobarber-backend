...
export default class UpdateUserProfileService {
  ...
  public async execute({ userId, email, name, oldPassword, password }: IRequest): Promise<User> {
    ...
    if (password) {
      ...
      const oldPasswordIsWrong = !(await this.hashProvider.compareHash(oldPassword, user.password))
